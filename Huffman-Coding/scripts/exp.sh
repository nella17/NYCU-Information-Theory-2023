...
for i in {1..8}; do
  ...
  (name="basic.$k.40MB" ARGS="-t basic -b $k -s 41943040 -v --no-time" ./test-report.sh > "log/exp-basic-$k-40MB.log" 2>&1)
done

(name="adaptive.8" ARGS="-t adaptive -b 8 -v --no-time" ./test-report.sh > "log/exp-adaptive-8.log" 2>&1)
